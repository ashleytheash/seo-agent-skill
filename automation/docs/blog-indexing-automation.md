...
> **Bundle layout:** Scripts live in [`automation/scripts/blog-indexing/`](../scripts/blog-indexing/). Workflows live in [`automation/workflows/`](../workflows/). Copy them into a product repo as described in [`automation/README.md`](../README.md). Example status outputs: [`automation/docs/examples/`](examples/).
...
doc is its concrete implementation in `seo-agent-skill/automation` (reference: `nexu-io/open-design`).

## What is automated

| Trigger | Job | Outcome |
|---|---|---|
| `landing-page-ci` / `landing-page-ci-seo` | `lint-blog-seo.ts` + `check-blog-url-changes.ts` | Changed posts are checked for frontmatter, internal/external links, rendered canonical/JSON-LD/OG metadata, and slug delete/rename redirects before they can merge. |
...
All scripts live in `automation/scripts/blog-indexing/` (product: `apps/landing-page/scripts/blog-indexing/`) and run
...
4. Confirm the existing `BOT_APP_ID` and `BOT_APP_PRIVATE_KEY` secrets
   ...
   and `issues:write` for `seo-agent-skill/automation` (reference: `nexu-io/open-design`) (already configured).
...
- PR opens → `landing-page-ci` / `landing-page-ci-seo` runs SEO lint and URL-change guards. A
...
- `automation/workflows/blog-indexing-on-deploy.yml`
- `automation/workflows/blog-indexing-monitor.yml`
- `automation/workflows/blog-3day-report.yml`
